...
public sealed class CoverLetterTemplateRenderer : ICoverLetterTemplateRenderer
{
    ...
    public CoverLetterTemplateRenderer(IHostEnvironment environment, IConfiguration configuration)
    {
    ...
    public async Task<CoverLetterTemplateRenderResult> RenderAsync(string applicationJson, CancellationToken cancellationToken = default)
    {
        ...
        var htmlTemplatePath = ApplyAiAssetPathResolver.ResolveCatalogPath(
            _configuration,
            _environment,
            "Templates/cover-letter-template.html");
        var cssTemplatePath = ApplyAiAssetPathResolver.ResolveCatalogPath(
            _configuration,
            _environment,
            "Templates/cover-letter-template.css");

        var htmlTemplate = await ReadRequiredTextAsync(htmlTemplatePath, cancellationToken);
        var stylesheetText = await ReadRequiredTextAsync(cssTemplatePath, cancellationToken);
        ...
        var budgetMetrics = CoverLetterContentMetrics.CalculateBudgetMetrics(
            renderedParagraphs,
            CoverLetterContentMetrics.DefaultEstimatedCharactersPerLine);
        var mainContentCharacterCount = budgetMetrics.VisibleCharacterCount;
        var withinMainContentLimit = budgetMetrics.BudgetUsage <= CoverLetterContentMetrics.DefaultMaxMainContentCharacters;
        if (!withinMainContentLimit)
        {
            warnings.Add(
                $"Den synlige hovedtekst er {mainContentCharacterCount} rå tegn, men {budgetMetrics.ParagraphBreakCount} paragrafskift og {budgetMetrics.ExplicitLineBreakCount} interne linjeskift løfter det effektive template-forbrug til {budgetMetrics.BudgetUsage} mod maksimum {CoverLetterContentMetrics.DefaultMaxMainContentCharacters}. PDF-layoutet kan blive klippet i højden.");
        }
        ...
        var htmlDocument = htmlTemplate
            ...
            .Replace("{{stylesheet_href}}", WebUtility.HtmlEncode("cover-letter.css"), StringComparison.Ordinal)
