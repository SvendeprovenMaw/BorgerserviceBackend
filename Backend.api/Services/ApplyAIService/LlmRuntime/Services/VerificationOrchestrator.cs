...
public sealed class VerificationOrchestrator : IVerificationOrchestrator
{
    ...
    private static HashSet<string> ExtractIds(string? documentJson, string arrayPropertyName, string idPropertyName)
    {
        ...
        try
        {
            ...
            if (!document.RootElement.TryGetProperty(arrayPropertyName, out var items) || items.ValueKind != JsonValueKind.Array)
            {
                return new HashSet<string>(StringComparer.Ordinal);
            }

            return items.EnumerateArray()
                .Select(item => GetString(item, idPropertyName))
                .Where(value => !string.IsNullOrWhiteSpace(value))
                .ToHashSet(StringComparer.Ordinal);
        }
        catch (JsonException)
